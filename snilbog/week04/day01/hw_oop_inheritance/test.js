...
////// TEST PHASE 1 - Motorcycle /////////////////////////////////////////

console.log('Testing motorcycle constructor');
var myBike = new Motorcycle('Honda', 'Shadow', 2005, 'Black', 2);
assert.strictEqual(myBike instanceof Motorcycle, true, 'Motorcycle should be an instance of Motorcycle');
assert.strictEqual(myBike instanceof Car, true, 'Motorcycle should be an instance of Car');
assert.strictEqual(myBike.make,'Honda','Constructor did not set make.');
assert.strictEqual(myBike.model,'Shadow','Constructor did not set model.');
assert.strictEqual(myBike.year,2005,'Constructor did not set year.');
assert.strictEqual(myBike.color,'Black','Constructor did not set color.');
assert.strictEqual(myBike.seats,2,'Constructor did not set seats.');

var myBike = new Motorcycle('Honda', 'Shadow', 2005, 'Black', 4);
assert.strictEqual(myBike.seats,2,'Constructor did not limit seats to 2.');
success();

console.log('trying a wheelie');
assert.equal(typeof(myBike.wheelie),'function','there is no wheelie function');
assert.strictEqual(typeof(myBike.wheelie()),'boolean','Wheelie function should return a boolean')
assert.strictEqual(myBike.wheelie(),false,'Should not be able to wheelie unless the bike is running.')
myBike.start();
assert.strictEqual(myBike.wheelie(),true,'Should not be able to wheelie unless the bike is running.')
success('MOTORCYCLE COMPLETE');

//testing instance of
console.log('Testing MOTORCYCLE!');
var myMotorcycle = new Motorcycle("Harley", "Harley 5x", 2017, "black", 2, 1);
assert.equal(myMotorcycle instanceof Motorcycle, true, "it is an instanceof motorcycle");
assert.equal(myMotorcycle instanceof Car, true, "it is an instanceof Motorcycle");
success('it worked');


////// Add tests for Truck below here /////////////////////////////////////////

var myTruck = new Truck('Ford', 'F250', '1999', 'magenta');
assert.strictEqual(myTruck.seats, 3, 'Truck seats should equal 3');
assert.strictEqual(myTruck.capacity, 0, 'Truck capacity');
